...
class Reports:
    ...
    def generate_customer_report(self):
        self.client_report = canvas.Canvas('cliente.pdf')
        ...
        # Criando o título do pdf
        self.client_report.setFont("Helvetica-Bold", 24)
        self.client_report.drawString(200, 790, 'Ficha do Cliente')

        # Criando o body do pdf
        self.client_report.setFont("Helvetica", 18)
        self.client_report.drawString(50, 700, 'Código: ')
        self.client_report.drawString(50, 670, 'Nome: ')
        self.client_report.drawString(50, 640, 'Telefone: ')
        self.client_report.drawString(50, 610, 'Cidade: ')

        self.client_report.setFont("Helvetica", 18)
        self.client_report.drawString(150, 700, self.code_report)
        self.client_report.drawString(150, 670, self.name_report)
        self.client_report.drawString(150, 640, self.telephone_report)
        self.client_report.drawString(150, 610, self.city_report)

        self.client_report.rect(20, 550, 550, 3, fill=True, stroke=False)

        self.client_report.showPage()
        self.client_report.save()
        self.print_client()
...
class Funcs:
    ...
    def change_client(self):
        ...
        self.cursor.execute(""" UPDATE clientes SET nome_cliente = ?, telefone = ?, cidade = ?
                    WHERE cod = ?""", (self.name, self.telephone, self.city, self.code))
        # /
        self.conn.commit()
...
class Application(Funcs, Reports):
    ...
    def first_frame_widgets(self):
        ...
        self.btn_change = Button(self.firstFrame, text='Mudar', bd=3, bg=config['cor4'], fg='white',
                                 font=config['font'], command=self.change_client)
    ...
    def menus(self):
        ...
        def quit(): self.main_window.destroy()

        menu_bar.add_cascade(label='Opções', menu=file_menu)
        menu_bar.add_cascade(label='Relatório', menu=file_menu2)

        file_menu.add_command(label='Sair', command=quit)
        file_menu.add_command(label='Limpar', command=self.clear_screen)
        file_menu2.add_command(label='Relatórios', command=self.generate_customer_report)
